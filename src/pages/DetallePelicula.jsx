...
import PeliculaDetalle from "../components/Detalle";
...
const DetallePelicula = () => {
  ...
  const tipo =
    isMovieParam === "true" || isMovieParam === null ? "movie" : "tv";
  ...
  const getMovie = async (id) => {
    try {
      ...
      const response = await axios.get(
        `${import.meta.env.VITE_API_URL}${tipo}/${id}`,
        {
          params: {
      ...
      if (data && data.videos && data.videos.results.length > 0) {
        const key = data.videos.results.find(
      ...
      if (data && data !== undefined) {
        const response2 = await axios.get(
          `${import.meta.env.VITE_API_URL}${tipo}/${id}/similar`,
          {
            params: {
              api_key: import.meta.env.VITE_API_KEY,
            },
  ...
  return (
    <>
      <button onClick={regresarAnterior}>Regresar</button>
      <PeliculaDetalle
        data={data}
        loading={loading}
        tipo={tipo}
